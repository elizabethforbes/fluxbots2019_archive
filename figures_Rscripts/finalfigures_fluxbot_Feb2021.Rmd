...
dens_g <- ggplot(fluxdat_final, aes(x=flux_umol_m2_sec, y=0.3))+
  ...
  # add mean line (not doing this for now bc mean with outliers is HUGE)
...
Also going to make a figure exploring the 'rotating head' data, (NO OS1 data from October 2019).
```{r rotatinghead}
...
summary(rotation_aov) # p = 0.00157 **
# post-hoc Tukey test to ID significant differences between rotations:
TukeyHSD(rotation_aov)
# significant difference between rotation 1 and 4: p = 0.0015295 (n = 22 vs. 40)
# borderline difference between rotation 1 and 5: p = 0.0533650 (n = 22 vs. 43)
# significant difference between rotation 2 and 4: p = 0.0114565 (n = 44 vs. 40)
...
summary(rotation_aov2) # p = <2e-16 ***
#post-hoc Tukey:
TukeyHSD(rotation_aov2)
# several rotational chunks are not significantly different from each other but that is to be expected looking at the plot (e.g. chunk 4 and 8)
```
...
```
Below plot looks at distribution of regressions with quadratic beta vs. linear beta, in same plot as the above plot:
```{r}
...
```{r}
# # percent error
# fluxdat_final$percent_error <- (abs((fluxdat_final$flux_max_umol_m2_sec-fluxdat_final$flux_min_umol_m2_sec)/fluxdat_final$flux_umol_m2_sec))*100
...
# Okay now put them together with an inset:
rel1 + annotation_custom(ggplotGrob(rel2), xmin=-25, xmax=-13,
                       ymin=4500, ymax = 10000)
```
...
# Okay now put them together with an inset:
rel3 + annotation_custom(ggplotGrob(rel4), xmin=23, xmax=45,
                       ymin=4500, ymax = 10000)
```
...
EXTRA
#########
Also a distribution plot of the beta values per bot:
```{r}
...
# plot:
g <- ggplot(fluxdat_beta_long, aes(x=beta_value, group=beta_type, fill=beta_type))+
...
g + scale_fill_discrete(name = NULL,
                        breaks = c("1st_order_beta_0", "2nd_order_beta_0"),
                        labels = c("linear regression beta", "quadratic regression beta"))
```
