...
def e_star(T):
	...
	return 0.61078 * exp((17.27 * T)/(T+ 273.3))
...
def order_n_fit(x, y, n=1):
	...
	if n == 1:
		fitfunc = lambda params, x: params[0] * x	#create fitting function of form y = m*x (no b)
	elif n == 2:
	...
	init_p = np.array(tuple(param_initial))  	#bundle initial values in initial parameters
	...
	syx2 = sse/(n_obs-n_params) # variance of y(x), predicted values of y at x versus actual values of y at x (using sse)
	sm2 = syx2 / ssxx
...
class Fluxbot():
	...
	def __init__(self,
		...
		self.sample_interval = sample_interval  # This is our rounding interval for fluxbots.
		...
		# 		AREA=FLUXBOT_AREA))
		self.area = area
		self.df = self.read_data(filename=filename)
		self.events, self.bad_events = self._get_events(event_numbers=event_numbers)
		self.output_filename = self.filename.split('.')[0].replace(" ","_") + '_output' + '_' + str(self.smoothing_interval) + '.csv'
	...
	def _parse_filename(self, file):
		...
		[dir_name, file_name] = file.split('/')

		# 2. The block is the first letter of the filename
		self.block = blocks[file_name[0]]

		# 3. Split the file_name to get the treatment.
		[treatment, remainder] = file_name.split(' ')
		self.treatment = treatment[1:] # Treament is the characters after Block
	...
	def read_data(self, filename=None):
		...
		df['Obs_Interval'] = round((df['StateChange'].cumsum()/0.9)).astype('int')
		df['Flux_Event'] = 0
		...
		df['CO2_conc'] = self.gas_ppm_to_conc(df['Raw.CO2.PPM'], df['Pressure'],  df['Temp'])
	...
	def _air_mol_volume(rho_a, volume=FLUXBOT_VOLUME):
		...
		return mol_density * (volume * 0.000001)  # Return moles of air in volume
	...
	@staticmethod
	def gas_ppm_to_conc(gas_ppm, P, T, g_per_mol=44.009):
		"""
		...
		gas_conc = mol_gas_m3 * (g_per_mol / 1000)  # CO2 concentration in kg/m^3
	...
	def gas_ppm_to_conc_CIRAS(gas_ppm, P, T, g_per_mol=44.009):
		...
		gas_conc_CIRAS = (gas_ppm) * (g_per_mol/molarV) * (273.15/(T+273.15)) * (P/atm) * (1e-6)
...
class Event():
	...
	def __init__(self, df, number=None, smoothing_interval=20):
		...
		self.number = number
		self.smoothing_interval = smoothing_interval
		self.smoothed_variables = ['Raw.CO2.PPM', 'CO2_mass', 'CO2_conc']
		...
		try:
			self.timestamp_raw = min(self.data.index) # Raw timestamp that the event started
			...
			self.timestamp_30_min = min(self.data.index.round('30min')) # Rounded timestamp to nearest 30 min
		except:
	...
	def determine_fits(self, variable='CO2_mass'):
		...
		"""
		try:
			T = pd.Series((self.data.index - self.data.index.min()).total_seconds())
	...
	def assign_change_in_CO2(self, method='max', b=None):
		...
		try:
			self.duration = (self.data.index.max() - self.data.index.min()).seconds
		except:
	...
	def output(self):
		...
		event_data['avg_pressure_hPa'] = '{:5.1f}'.format(self.data.Pressure.mean())
		event_data['avg_rel_humidity'] = '{:5.1f}'.format(self.data.Humidity.mean())
		...
		event_data['1st_order_beta_0_max'] = '{:5.2E}'.format(_1st_order_beta_0_max)
		event_data['1st_order_beta_0_min'] = '{:5.2E}'.format(_1st_order_beta_0_min)
		event_data['1st_order_r_sq'] = '{:5.3f}'.format(_1st_order_r_sq)
		event_data['2nd_order_beta_0'] = '{:5.3E}'.format(_2nd_order_beta_0)
		event_data['2nd_order_beta_0_max'] = '{:5.2E}'.format(_2nd_order_beta_0_max)
		event_data['2nd_order_beta_0_min'] = '{:5.2E}'.format(_2nd_order_beta_0_min)
		event_data['2nd_order_beta_1'] = '{:5.2E}'.format(_2nd_order_beta_1)
		event_data['2nd_order_beta_1_max'] = '{:5.2E}'.format(_2nd_order_beta_1_max)
		event_data['2nd_order_beta_1_min'] = '{:5.2E}'.format(_2nd_order_beta_1_min)
		...
		if smoothing_interval:
			if smoothed_variables:
				for variable in smoothed_variables:
					df[variable] = Event.smooth(df[variable], smoothing_interval=smoothing_interval)
